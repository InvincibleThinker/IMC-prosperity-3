...
class Trader:
    ...
    def kelp_strategy(self, product, order_depth, position, position_limit) -> List[Order]:
        ...
        if order_depth.sell_orders and order_depth.buy_orders:
            ...
            # Dynamic VWAP window based on recent volatility
            if len(self.kelp_prices) >= 2 and statistics.stdev(self.kelp_prices[-20:]) > 4:
                vwap_window = 8
            else:
                vwap_window = 16
            ...
            # Passive orders for better entry/exit
            baaf = min([p for p in order_depth.sell_orders if p >
